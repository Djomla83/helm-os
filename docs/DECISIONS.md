...
[disposition of 2026-09-20](#helm-launch-p4-independent-findings-disposition). The correction was
made; three publications failed, on evidence that is preserved unchanged, and the fourth passed
every load-bearing hosted Linux gate on its first natural run, so P4 was
[accepted on 2026-09-20](#helm-launch-p4-accepted). On the same day the owner authorised the final
**HELM-LAUNCH P5** slice — the Level 4 regressions, the published receipt evidence contract,
current-truth documentation and CI hardening, with no new architecture; see the
[decision of 2026-09-20](#helm-launch-p5-authorised). P5 was implemented, reviewed by one fresh
independent whole-crate review, corrected once for `P5R-01` and independently rereviewed. On
2026-09-21 the owner **accepted P5 and product-accepted the complete helm-launch 0.1 module** at
...
<a id="helm-launch-p5-accepted"></a>
<a id="helm-launch-0-1-product-accepted"></a>

### Owner decision 2026-09-21 — **HELM-LAUNCH P5 ACCEPTED**; **helm-launch 0.1 PRODUCT-ACCEPTED**; integrated into main

**`HELM_LAUNCH_0_1_PRODUCT_ACCEPTED`.** The repository owner, Djomla83, accepts **HELM-LAUNCH P5**,
the final helm-launch 0.1 implementation slice authorised by the
[decision of 2026-09-20](#helm-launch-p5-authorised), and accepts the **complete helm-launch 0.1
module** as a product. This closes the implementation line opened by Accepted
[ADR-0024](adr/ADR-0024-launch-authority.md) and the accepted
[productization plan](implementation/HELM-LAUNCH-PRODUCTIZATION-PLAN.md). It changes **no** accepted
architecture and promotes **no** claim beyond the reviewed 0.1 contract.
...
| Next owner gate | **G2 VISUAL KICKOFF** |

#### 1. Evidence this acceptance rests on

The P5 stop condition is satisfied. P5 was implemented, then reviewed by **one fresh independent
review of the whole crate** at `6e6057c`, by a reviewer who authored neither P5 commit. That review
raised one finding requiring correction, `P5R-01`, on the receipt evidence contract; the correction
is `5745962` and was **independently rereviewed** at `dd92a85`, which is the accepted head.

Final hosted validation on the accepted head passed on the **first natural run, attempt 1**, with
no dispatch, no retry, no rerun and no replacement:

| Workflow | Run | Attempt | Event | Conclusion |
|---|---|---|---|---|
| `helm-launch` | `35538669764` | 1 | `push` | **SUCCESS** |
| `workspace` | `35538669783` | 1 | `push` | **SUCCESS** |

#### 2. What this acceptance does not promote

Product acceptance promotes **no** claim beyond the reviewed 0.1 contract. Every nonclaim stands
exactly as reviewed:

* a clean status EOF remains **`Indeterminate(StatusEofWithoutRecord)`**; there is **no
  `ExecSucceeded`** state and no exec-success claim;
* the process-group sweep is **best-effort cleanup, never containment**; `group_sweep = issued`
  says one call was issued, never that a descendant received it or died;
* a `LaunchReceipt` is **data only**: zero execution authority, fabricatable, unsigned, and
  carrying **no authenticity and no provenance**;
* a digest asserts **byte identity only**;
* there is **no Wine, no Proton, no orchestrator, no sandbox, no cgroups and no async launch**;
* the **N3 privileged-transition claim stays UNVALIDATED**.

Schema, API and limits remain experimental and unstabilised, `publish = false`, and this is **not a
release**.

#### 3. Findings carried deliberately

Acceptance **carries** the remaining nonblocking MINOR and BACKLOG findings rather than clearing
them. This is intentional, and they are **not** to be cleaned up as part of the integration merge:

`P5R-02`, `P5R-03`, `P5R-04`, `P5R-05`, `P5R-06`;
`P5R01R-M1`, `P5R01R-M2`, `P5R01R-M3`;
`P5R01R-B1`;
`P4A-03`;
`P1-TEST-01`;
`P3R-03` / `F-P4-M2`;
`P3R-04`, `P3R-05`, `P3R-06`, `P3R-07`, `P3R-08`, `P3R-12`, `P3R-16`, `P3R-17`, `P3R-18`,
`P3R-19`;
`P3R-13` (**BACKLOG**);
`P3R21-M1`.
...
The reviewed milestone branch `docs/helm-launch-architecture` and `main` had diverged from the
merge base `5cc56384257a2ec1f2a2a9c64f7f4328da6cef2e`: five commits on main, 141 on the milestone.
The milestone is integrated by **one ordinary two-parent merge commit** on main, first parent
`501a7fa95c4884da4fec9a20a512c2d63f2b30cc` and second parent `dd92a85`. **No history is rewritten**,
no branch is rebased, nothing is squashed or cherry-picked, and every published SHA and every piece
of hosted evidence stays valid.

**The merge commit is not the reviewed implementation head.** The reviewed product and evidence head
remains `dd92a85fcf0cc35306123ef2dc39adb148ddc616`. The merge carries the reviewed product tree
unchanged: every file under `crates/helm-launch/src/`, `crates/helm-launch/tests/`, the
`helm-launch` workflow, the `helm_launch_*` tools and their tests, `Cargo.toml` and `Cargo.lock` are
object-identical to that head, and only `crates/helm-launch/README.md` and the status documents
differ, by acceptance status prose alone.

Main-only history survives unchanged: [local state layout](operations/LOCAL-STATE.md) is preserved
as main wrote it, and the consumed Trial #2 and Trial #3 dispatchers keep their exact blobs. The
**frozen trial history is untouched** — no dispatcher is reactivated, no trial conclusion or attempt
history is changed, and Trial #3 stays historically `MECHANISM_REJECTED`, which does not conflict
with product acceptance of the supported launch mechanism.

#### 5. Authority after this decision

**No further helm-launch implementation slice is authorised.** No Trial #4 is authorised, and none
